<x-admin-layout
    title="Áreas · {{ $client->name }} | Diagramas"
    :breadcrumbs="[
        ...
        <div class="flex items-start justify-between gap-4">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                <button id="btn-rebuild"
                    ...
                    <svg id="rebuild-icon" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        @else
            <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <span class="text-xs text-gray-400">{{ $batches->count() }} área{{ $batches->count() !== 1 ? 's' : '' }}</span>
                </div>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    @if($batch->fail_count > 0)
                                        <div class="text-xs text-red-500 mt-0.5">{{ $batch->fail_count }} con error</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @php
                                        $pct = $batch->switches_count > 0
    ...
    function rebuildConnections() {
        ...
        .catch(() => {
            ...
            toast.className = 'px-4 py-3 rounded-lg border text-sm font-medium bg-red-50 border-red-200 text-red-800';
            toast.classList.remove('hidden');
        })
        .finally(() => {
            btn.disabled = false;
