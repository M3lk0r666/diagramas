<x-admin-layout title="Inventario de Red | Diagramas" :breadcrumbs="[['name' => 'Dashboard', 'href' => route('dashboard')], ['name' => 'Inventario']]">
    ...
    <div class="space-y-5">
        ...
        <div class="flex items-center justify-between gap-4 flex-wrap">
            <div class="flex items-center gap-3 min-w-0">
                <a href="{{ route('admin.hub.index') }}"
                   class="shrink-0 inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium
                    ...
                    <i class="ri-arrow-left-s-line text-base"></i> Clientes
                </a>
                <div>
            ...
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <button id="btn-gen-images" onclick="generateAllImages()"
                    class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            @php
                $stats = [
                    [
                        'label' => 'Switches',
                        'value' => number_format($totalSwitches),
                        'icon' => 'ri-server-line',
                        'color' => 'blue',
                    ],
                    [
                    ...
                    [
                        'label' => 'Áreas',
                        'value' => number_format($totalAreas),
                        'icon' => 'ri-node-tree',
                        'color' => 'emerald',
                    ],
                    [
                    ...
                    [
                        'label' => 'Sin resolver',
                        'value' => number_format($unresolvedConns),
                    ...
                    <tbody id="inv-tbody">
                        ...
                        @foreach ($grouped as $batchId => $batchRows)
                            @php
                                ...
                                $topoUrl =
                                    $clientId && $batchId ? route('admin.areas.topology', [$clientId, $batchId]) : null;
                            @endphp
                            ...
                            <tr class="inv-group-header border-t-2 border-indigo-100 bg-indigo-50/70
                                ...
                                data-group="{{ $batchId }}" data-client="{{ $clientId ?? '' }}" data-expanded="1"
                                ...
                                <td colspan="7" class="px-2 py-2.5">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        <span
                                            class="text-xs bg-indigo-100 text-indigo-600 px-2 py-0.5 rounded-full font-medium ml-1">
                                ...
                                <td class="px-4 py-2.5 text-center w-16">
                                    @if ($topoUrl)
                                        <a href="{{ $topoUrl }}" onclick="event.stopPropagation()"
                                            title="Ver topología"
                            ...
                            @foreach ($batchRows as $i => $sw)
                                ...
                                <tr class="inv-row inv-body-{{ $batchId }} hover:bg-blue-50/40 transition-colors"
                                    ...
                                    data-client="{{ $sw['client_id'] }}" data-area="{{ $sw['batch_id'] }}"
                                    data-role="{{ $sw['role'] }}">
                                    ...
                                    <td class="px-4 py-2.5 text-center">
                                        <span class="inline-flex items-center justify-center gap-1.5"
                                            ...
                                            <span
                                                class="text-xs text-gray-500 hidden xl:inline">{{ $roleLabel }}</span>
                                        </span>
                                    </td>
    ...
    @push('js')
        <script>
            ...
            async function generateAllImages() {
                ...
                btn.disabled = false;
                btn.innerHTML =
                    `<i class="ri-check-line text-sm"></i> ¡Listo! (${total - errors} generadas${errors ? ', ' + errors + ' con error' : ''})`;
                setTimeout(() => {
                ...
                if (errors === 0) {
                    ...
                    if (confirm(
                            `Se encolaron ${total} generaciones. Las imágenes estarán disponibles en el Ensamblador en breves segundos.\n\n¿Abrir el Ensamblador ahora?`
                            )) {
