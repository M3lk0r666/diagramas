<x-admin-layout :title="$batch->name . ' — Diagrama | Diagramas'" :breadcrumbs="[
    ...
    ['name' => 'Diagramas',  'href' => route('admin.home')],
            ...
            @if($hasImage)
                <div class="p-2 text-center bg-gray-50 dark:bg-gray-900">
                    <img src="{{ route('admin.batches.diagram.image', $batch) }}?t={{ time() }}"
                         ...
                         class="max-w-full mx-auto rounded shadow-sm cursor-zoom-in"
                         id="diagram-img"
                         onclick="toggleZoom(this)">
                </div>
                <p class="text-center text-xs text-gray-400 py-2">
                    Haz clic en la imagen para ampliar · <a href="{{ route('admin.batches.diagram.image', $batch) }}" target="_blank" class="text-blue-500 hover:underline">Abrir en nueva pestaña</a>
                </p>
            @else
                {{-- Sin imagen todavía --}}
                <div class="py-20 text-center space-y-4">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16 text-gray-300 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach(['core','backbone','distribution','access'] as $role)
                        @if($nodesByRole->has($role))
                            @php
                                $colors = ['core'=>'bg-blue-900 text-white','backbone'=>'bg-blue-600 text-white','distribution'=>'bg-cyan-600 text-white','access'=>'bg-green-600 text-white'];
                                $labels = ['core'=>'Core','backbone'=>'Backbone','distribution'=>'Distribución','access'=>'Acceso'];
                            @endphp
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium {{ $colors[$role] }}">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 px-6 py-5">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="font-semibold text-gray-700 dark:text-gray-200 text-sm">
                        Clústeres por hub
                        <span class="ml-2 text-xs font-normal text-gray-400">({{ count($batch->topology_clusters) }} hubs)</span>
                    </h3>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($batch->topology_clusters as $cluster)
                        @php
                            ...
                            $roleColors = ['core'=>'bg-blue-900','backbone'=>'bg-blue-600','distribution'=>'bg-cyan-600'];
                        ...
                        @if($imgUrl)
                            <div class="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden flex flex-col
                                ...
                                {{-- Thumbnail --}}
                                <a href="{{ $imgUrl }}" target="_blank" class="block bg-gray-50 dark:bg-gray-900 overflow-hidden"
                                   title="Ver {{ $hubLabel }} en detalle">
                                    <img src="{{ $imgUrl }}?t={{ time() }}"
                                         alt="Clúster {{ $hubLabel }}"
                                         class="w-full object-cover"
                                         style="max-height:180px;object-fit:cover;object-position:center;"
                                         loading="lazy">
                                </a>
                                {{-- Card footer --}}
                                <div class="px-3 py-2 flex items-center gap-2 border-t border-gray-100 dark:border-gray-700">
                                    <span class="inline-block w-2 h-2 rounded-full {{ $roleColor }} flex-shrink-0"></span>
                                    <div class="min-w-0 flex-1">
                                        <p class="text-xs font-semibold text-gray-700 dark:text-gray-200 truncate"
                                           title="{{ $hubLabel }}">{{ $hubLabel }}</p>
                                        <p class="text-xs text-gray-400">{{ $nodeCount }} nodos · {{ $edgeCount }} enlaces</p>
                                    </div>
                                    <a href="{{ $imgUrl }}"
                                       download="{{ Str::slug($hubLabel) }}_cluster.png"
