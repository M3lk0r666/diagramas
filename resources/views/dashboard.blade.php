...
<div class="space-y-6 pb-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Resumen general del sistema de gestión de red · {{ now()->isoFormat('D [de] MMMM, YYYY') }}</p>
        </div>
        <div class="flex gap-2 flex-wrap">
            ...
            <a href="{{ route('admin.assembler.create') }}"
               class="inline-flex items-center gap-1.5 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors">
                <i class="ri-layout-grid-line"></i> Nuevo diagrama
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        @php
            $kpis = [
                ['label'=>'Clientes',      'value'=>$totalClients,              'icon'=>'ri-building-2-line',     'color'=>'blue',    'href'=>route('admin.clients.manage.index')],
                ['label'=>'Áreas',         'value'=>$totalBatches,              'icon'=>'ri-folder-zip-line',     'color'=>'violet',  'href'=>route('admin.areas.index')],
                ['label'=>'Switches',      'value'=>$totalSwitches,             'icon'=>'ri-cpu-line',            'color'=>'cyan',    'href'=>route('admin.switches.index')],
                ['label'=>'Conexiones',    'value'=>number_format($totalConnections), 'icon'=>'ri-git-branch-line','color'=>'emerald','href'=>route('admin.topology.index')],
                ['label'=>'Diagramas',     'value'=>$totalProjects,             'icon'=>'ri-layout-grid-line',    'color'=>'orange',  'href'=>route('admin.assembler.index')],
                ['label'=>'Archivos cfg',  'value'=>$totalFiles,                'icon'=>'ri-file-code-line',      'color'=>'slate',   'href'=>route('admin.inventario.index')],
            ];
            $colorMap = [
                'blue'    => ['bg'=>'bg-blue-50',    'ibg'=>'text-blue-600',    'val'=>'text-blue-700',    'border'=>'border-blue-100',    'ring'=>'hover:ring-blue-200'],
                'violet'  => ['bg'=>'bg-violet-50',  'ibg'=>'text-violet-600',  'val'=>'text-violet-700',  'border'=>'border-violet-100',  'ring'=>'hover:ring-violet-200'],
                'cyan'    => ['bg'=>'bg-cyan-50',     'ibg'=>'text-cyan-600',    'val'=>'text-cyan-700',    'border'=>'border-cyan-100',    'ring'=>'hover:ring-cyan-200'],
                'emerald' => ['bg'=>'bg-emerald-50',  'ibg'=>'text-emerald-600', 'val'=>'text-emerald-700', 'border'=>'border-emerald-100', 'ring'=>'hover:ring-emerald-200'],
                'orange'  => ['bg'=>'bg-orange-50',   'ibg'=>'text-orange-600',  'val'=>'text-orange-700',  'border'=>'border-orange-100',  'ring'=>'hover:ring-orange-200'],
                'slate'   => ['bg'=>'bg-slate-50',    'ibg'=>'text-slate-500',   'val'=>'text-slate-700',   'border'=>'border-slate-100',   'ring'=>'hover:ring-slate-200'],
            ];
        @endphp
        ...
        <a href="{{ $k['href'] }}"
           class="relative group bg-white border {{ $c['border'] }} rounded-xl p-4 hover:shadow-md hover:ring-2 {{ $c['ring'] }} transition-all duration-200">
            <div class="flex items-start justify-between mb-3">
                ...
                <i class="ri-arrow-right-up-line text-gray-300 group-hover:text-gray-500 text-sm transition-colors mt-1"></i>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
            ...
            @php
                $statusItems = [
                    ['label'=>'Completadas', 'val'=>$batchCompleted,  'bar'=>'bg-emerald-500', 'badge'=>'bg-emerald-100 text-emerald-700'],
                    ['label'=>'Fallidas',    'val'=>$batchFailed,     'bar'=>'bg-red-500',     'badge'=>'bg-red-100 text-red-700'],
                    ['label'=>'Procesando',  'val'=>$batchProcessing, 'bar'=>'bg-blue-500',    'badge'=>'bg-blue-100 text-blue-700'],
                    ['label'=>'Pendientes',  'val'=>$batchPending,    'bar'=>'bg-amber-400',   'badge'=>'bg-amber-100 text-amber-700'],
                ];
            @endphp
            <div class="space-y-3">
                ...
                <div>
                    <div class="flex justify-between items-center mb-1">
                        ...
                        <span class="text-xs font-bold px-2 py-0.5 rounded-full {{ $s['badge'] }}">{{ $s['val'] }}</span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-1.5">
                        @if($totalBatches > 0)
                        <div class="{{ $s['bar'] }} h-1.5 rounded-full" style="width:{{ round($s['val']/$totalBatches*100) }}%"></div>
                        @endif
                    </div>
                </div>
                @endforeach
        ...
        <div class="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
            ...
            <div class="mt-4 pt-3 border-t border-gray-100">
                ...
                <div class="w-full bg-gray-100 rounded-full h-1.5">
                    <div class="bg-violet-500 h-1.5 rounded-full" style="width:{{ round($stackedSwitches/$totalSwitches*100) }}%"></div>
                </div>
            </div>
            @endif
        </div>
        ...
        <div class="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
            <div class="flex items-center justify-between mb-1">
                ...
                <a href="{{ route('admin.areas.index') }}" class="text-xs text-blue-600 hover:underline">Ver todo →</a>
            </div>
            ...
            @php
                ...
                $stLabel = ['completed'=>'OK','failed'=>'Error','processing'=>'Proc.','pending'=>'Pend.'];
            @endphp
            <div class="space-y-2.5">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-[10px] text-gray-400 truncate">{{ $batch->client->name ?? '—' }} · {{ $batch->updated_at->diffForHumans() }}</p>
                    </div>
                    <span class="flex-shrink-0 text-[10px] font-bold px-1.5 py-0.5 rounded-full {{ $stBadge[$batch->status] ?? 'bg-gray-100 text-gray-500' }}">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-50 text-xs">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-5 py-3">
                                <div class="flex items-center gap-2.5">
                                    <div class="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center flex-shrink-0">
                                        <span class="text-blue-600 font-bold text-xs">{{ strtoupper(substr($c->name,0,2)) }}</span>
                                    </div>
                                    <div>
                            ...
                            <td class="px-4 py-3 text-center font-bold text-gray-700">{{ $c->batches_count }}</td>
                            <td class="px-4 py-3 text-center">
                                <span class="font-semibold text-emerald-600">{{ $c->completed_batches }}</span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="font-semibold {{ $c->failed_batches > 0 ? 'text-red-500' : 'text-gray-300' }}">
                                    {{ $c->failed_batches }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="font-bold text-cyan-700">{{ number_format($c->switch_total) }}</span>
                            </td>
                            <td class="px-4 py-3 text-center">
        ...
        <div class="lg:col-span-2 bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
            <div class="flex items-center justify-between mb-1">
                ...
                <a href="{{ route('admin.assembler.index') }}" class="text-xs text-blue-600 hover:underline font-medium">Ver todos →</a>
            </div>
            ...
            <div class="space-y-2">
                ...
                @php
                    ...
                    $er = $isVect ? route('admin.assembler.vectorial',$proj) : route('admin.assembler.edit',$proj);
                @endphp
                <a href="{{ $er }}"
                   class="flex items-center gap-3 p-2.5 rounded-xl hover:bg-gray-50 transition-colors group border border-transparent hover:border-gray-100">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0
                        ...
                        <i class="{{ $isVect ? 'ri-node-tree text-indigo-600' : 'ri-image-line text-blue-600' }} text-lg"></i>
                    </div>
                    <div class="flex-1 min-w-0">
                    ...
                    <div class="flex-shrink-0 flex flex-col items-end gap-1">
                        ...
                        <i class="ri-arrow-right-s-line text-gray-300 group-hover:text-blue-400 text-sm transition-colors"></i>
                    </div>
                </a>
                @empty
    ...
    <div>
        ...
        <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3">
            @php
                $modules = [
                    ['label'=>'Subir archivos',  'icon'=>'ri-upload-cloud-2-line', 'color'=>'blue',   'href'=>route('admin.home'),                 'desc'=>'Cargar configs'],
                    ['label'=>'Inventario',       'icon'=>'ri-list-check-3',        'color'=>'slate',  'href'=>route('admin.inventario.index'),     'desc'=>'Lista de switches'],
                    ['label'=>'Clientes',         'icon'=>'ri-building-2-line',     'color'=>'blue',   'href'=>route('admin.clients.manage.index'), 'desc'=>'Gestión de clientes'],
                    ['label'=>'Switches',         'icon'=>'ri-cpu-line',            'color'=>'cyan',   'href'=>route('admin.switches.index'),       'desc'=>'Detalle switches'],
                    ['label'=>'Topología',        'icon'=>'ri-share-line',          'color'=>'green',  'href'=>route('admin.topology.index'),       'desc'=>'Red completa'],
                    ['label'=>'Áreas',            'icon'=>'ri-node-tree',           'color'=>'violet', 'href'=>route('admin.areas.index'),          'desc'=>'Por área/cliente'],
                    ['label'=>'Ensamblador',      'icon'=>'ri-image-2-line',        'color'=>'orange', 'href'=>route('admin.assembler.index'),      'desc'=>'Mis diagramas'],
                    ['label'=>'Nuevo Diagrama',   'icon'=>'ri-add-circle-line',     'color'=>'indigo', 'href'=>route('admin.assembler.create'),     'desc'=>'PNG o Vectorial'],
                    ['label'=>'GoJS',             'icon'=>'ri-git-branch-line',     'color'=>'teal',   'href'=>route('admin.topology.gojs.show'),   'desc'=>'Topología interactiva'],
                    ['label'=>'Topo. Custom',     'icon'=>'ri-settings-3-line',     'color'=>'rose',   'href'=>route('admin.topology.custom.show'), 'desc'=>'Filtros avanzados'],
                    ['label'=>'Clientes',         'icon'=>'ri-table-line',          'color'=>'slate',  'href'=>route('admin.clients.index'),        'desc'=>'Tabla de clientes'],
                    ['label'=>'Archi. Topo.',     'icon'=>'ri-folder-image-line',   'color'=>'orange', 'href'=>route('admin.areas.index'),          'desc'=>'Topologías por área'],
                ];
                $qc = [
                    'blue'   => 'bg-blue-50 text-blue-600 border-blue-100 hover:border-blue-300 hover:bg-blue-100',
                    'slate'  => 'bg-slate-50 text-slate-600 border-slate-100 hover:border-slate-300 hover:bg-slate-100',
                    'cyan'   => 'bg-cyan-50 text-cyan-600 border-cyan-100 hover:border-cyan-300 hover:bg-cyan-100',
                    'green'  => 'bg-green-50 text-green-600 border-green-100 hover:border-green-300 hover:bg-green-100',
                    ...
                    'indigo' => 'bg-indigo-50 text-indigo-600 border-indigo-100 hover:border-indigo-300 hover:bg-indigo-100',
